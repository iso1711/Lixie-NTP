...
#define SIX_DIGIT_CLOCK true   // 6 or 4-digit clock? (6 has seconds shown)
...
struct conf {
  int16_t time_zone_shift = 1;  //+1 for Central European Time CET (+2 for Central European Summer Time CEST)
...
void color_for_mode() {
  ...
  if (clock_config.current_mode == MODE_SOLID) {
    ...
    const uint8_t nixie_aura_level = 0; // 0-255
